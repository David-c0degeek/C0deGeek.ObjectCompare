...
public sealed class ExpressionCloner(ComparisonConfig config)
{
    ...
    private object? CloneObject(object? obj)
    {
        ...
        // Handle collections before creating a new instance
        if (metadata.IsCollection)
        {
            // Clone the collection
            var clonedCollection = CloneCollection(obj, type);

            // Add to circular reference tracker
            _circularReferenceTracker[obj] = clonedCollection;
    ...
    private void CloneObjectProperties(object source, object target)
    {
        ...
        foreach (var prop in metadata.Properties)
        {
            ...
            try
            {
                ...
                // Ensure we don't pass null for value types
                if (prop.PropertyType.IsValueType && clonedValue == null)
                {
                    clonedValue = Activator.CreateInstance(prop.PropertyType);
                }
